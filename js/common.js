...
var lnbFn = {
    ...
    init: function(t) {
		...
		$(".depth2 li").removeClass("arr");

		$lnb.find(".gnb > ul > li").on("mouseenter mouseleave",function(e){
			...
			if(e.type == "mouseenter"){
				...
				if ($this.find("div").hasClass("depth3")) { 
					$(".depth3").closest("li").addClass("arr")
				} else {
				}
				lnbFn.open();
...
function viewSch(e){
	var obj = $(e);
	var cont = obj.siblings(".schbox");

	cont.toggleClass("on");
	obj.toggleClass("close");
	$(".area_search_top").slideUp(200);
...
function pageInit(p,s){
  var pNum = p , sNum =s;
  var $warp = $("#wrap") , $head = $("#header"), $lnb = $("#lnb"), $smenu = $(".smenu");

  if( pNum > 0 && pNum <= $lnb.find(".gnbbox > .gnb > li").length ){
      $warp.addClass("dep2");
      LNB_FIRST_DEP = $lnb.find(".gnbbox > .gnb > li.mc"+pNum).addClass("on");
  }
}//pageInit
...
function box_seleect_ty1(){
    ...
    $(".box-select-ty1 .selectMenu>li>a").on('click',function(e){
        if(!$(this).parents(".box-select-ty1").hasClass("no_sel")){
            ...
            $(this).parents(".box-select-ty1").find(".selectVal a").empty().prepend(obj);
         }
    });
...
var openModal = null;
function closeSwitchModal(b){
    if(openModal == "alert") {
       alertPopup.close("switch");
    }else if(openModal == "layer") {
       layerPopup.close("switch");
    }
...
var layerPopup = {
	...
	open : function (id){
		...
		if(openModal != null){
			closeSwitchModal(openModal);
		}else{
			$("body").append("<div class='modal-cover'>");
			$(".modal-cover").animate({"opacity":.8},300);
		}

		if ($id.hasClass('alert')){
			$(".layerPopup.alert").css("top","-100%").stop().animate({"top":"60px", "opacity":1},200);
			openModal = "alert";
		} else {
			openModal = "layer";
		}
...
var alertPopup = {
	$modal : ".modal-cover",
	open : function (id,txt,func,style){
		var $msg = txt;
		...
		if (!$("#"+id).length > 0){
			$("#wrap").append("<div class='modal-cover alert'>");
			$(".modal-cover.alert").animate({"opacity":.8},300);

			$("body").append( $("<div id="+ id +" class='layerPopup alert'>") );

			alertDiv =  '<div class="popup">'
					 +   '<p class="t1">'+ $msg +'</p>'
					 +   '<div class="btn-bot">'
					 +		'<a href="/" class="btn-pk n blue b-close"><span>확인</span></a>'
					 +	  '</div>'
					 +	 '</div>';
			...
			setTimeout(function(){
				$(".layerPopup.alert").show().css("top","-100%").stop().animate({"top":"60px", "opacity":1},200);
				openModal = "alert";
...
$(function(){
	...
	$(".btn_menu").on("click", function(){
		var num = 200;
		if ($(".lnb").is(":visible")){
			$(".sidebar").stop().animate({"left": -(num)},200).addClass("on");
			$(".contents").stop().animate({"marginLeft": "60px"},200);
			$(".lnb").hide();
		} else {
			$(".sidebar").stop().animate({"left": 0},200).removeClass("on");
			$(".contents").stop().animate({"marginLeft": "260px"},200);
			$(".lnb").show();
		}
	});

	if(mobileSizeFlag()){//모바일일때
		...
		$(".sidebar").stop().animate({"left": "-200px"},200).addClass("on");
		$(".contents").stop().animate({"marginLeft": "60px"},200);
		$(".lnb").hide();
	}
		...
		if ($target.hasClass('img')){
			...
			if(window.FileReader){  
			  ...
			  reader.onloadend = function (e) {
				$target.siblings('.file_photobox').css({'background-image':'url("' + e.target.result + '")', 'background-size':'contain'});
			  }
			  if (file) {
			...
			// 추출한 파일명 삽입
			$target.siblings('.upload-name').show();
			$target.siblings('.upload-name').find("span").text(fileName);
	...
	//테이블 검색 더보기
	$(".btn-c-more").on("click", function(){
		var $this = $(this);
		var $next = $this.closest(".col").next(".col.view");

		if ($this.children("span").hasClass("on")){
			$this.children("span").removeClass("on").text("더보기");
			$next.slideUp(100);
		} else {
			$this.children("span").addClass("on").text("숨기기");
			$next.slideDown(150);
		}
	});
	...
	$(".tr_list").on("click", function(){
		if($(this).next(".tr_view").is(":visible")){
			$(this).next(".tr_view").hide();
		} else {
			$(".tr_view").hide();
			$(this).next(".tr_view").show();
		}
...
var fileUpload = {
	init : function(){
		this.input = $('.tbl_file_box .file');
		this.uploadArea = $('.tbl_file_box .upload-name');
		this.name = this.uploadArea.find('span');
		this.delBtn = $('.tbl_file_box .del-btn');
		this.fileName = null;
	...
	getName : function(){
		...
		this.input.on('change', function(){
			_self.fileName = this.files[0].name
			_self.uploadArea.css('display','inline')
			_self.setName( _self.fileName )
		})
	},
	...
	delete : function(){
		...
		this.delBtn.on('click', function(){
			_self.input.val('')
			_self.uploadArea.css('display','none')
		})
	}
}
...
//스트콜
function scrollDesign(id,position){
	var pos = position;
	$(id).each(function(){
...
$(window).load(function(){loadingEnd();})